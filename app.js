...
function renderCalendar(){
 ...
 let cells="";for(let i=0;i<start;i++)cells+='<div class="day muted"></div>';for(let d=1;d<=daysIn;d++){let date=new Date(y,m,d),items=state.assignments.filter(a=>a.due_at&&new Date(a.due_at).toDateString()===date.toDateString()),ex=state.exams.filter(a=>new Date(a.starts_at).toDateString()===date.toDateString());cells+=`<div class="day"><b>${d}</b>${items.slice(0,3).map(a=>{let c=state.courses.find(c=>c.id===a.course_id);return `<span class="cal-chip course-chip" style="--course-color:${esc(c?.color||"#d9d9d9")}">${esc(c?.code?c.code+" · ":"")}${esc(a.title)}</span>`}).join("")}${ex.slice(0,2).map(a=>{let c=state.courses.find(c=>c.id===a.course_id);return `<span class="cal-chip exam-chip course-chip" style="--course-color:${esc(c?.color||"#777")}">📝 ${esc(c?.code?c.code+" · ":"")}${esc(a.title)}</span>`}).join("")}</div>`}
